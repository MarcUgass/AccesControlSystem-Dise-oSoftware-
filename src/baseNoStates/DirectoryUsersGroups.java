...
public class DirectoryUsersGroups {
  ...
  public static void makeUsersGroups() {
    ...
    ArrayList<DayOfWeek> daysEmployee = new ArrayList<>(Arrays.asList(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.FRIDAY));

    Schedule scheduleEmployee = new Schedule(startDateEmployee, endDateEmployee,daysEmployee, startTimeEmployee, endTimeEmployee);

    ArrayList<String> actionsEmployee = new ArrayList<>(Arrays.asList(Actions.UNLOCK_SHORTLY, Actions.OPEN, Actions.CLOSE));

    ArrayList<Area> areasEmployee = new ArrayList<>(Arrays.asList(DirectoryAreas.findAreaById("ground_floor"), DirectoryAreas.findAreaById("floor1"), DirectoryAreas.findAreaById("exterior"), DirectoryAreas.findAreaById("stairs")));

    ArrayList<User> usersEmployee = new ArrayList<>(Arrays.asList(new User("Ernest", "74984"), new User("Eulalia" , "43295")));

    UserGroup employee = new UserGroup("employee", scheduleEmployee, actionsEmployee, areasEmployee, usersEmployee);

    userGroups.add(employee);

    // Managers
    LocalDate dateInicioManagers = LocalDate.of(YEAR, Month.SEPTEMBER, ONE.intValue());
    ...
    ArrayList<String> actionsManagers = new ArrayList<>(Arrays.asList(Actions.OPEN, Actions.CLOSE, Actions.LOCK, Actions.UNLOCK,
        Actions.UNLOCK_SHORTLY));

    ArrayList<Area> areasManagers = new ArrayList<>(Arrays.asList(DirectoryAreas.findAreaById("building")));
    ...
    Schedule scheduleAdmin = new Schedule(startDateAdmin, endDateAdmin, daysAdmin, startTimeAdmin, endTimeAdmin);
    ...
    ArrayList<Area> areasAdmin = new ArrayList<>(Arrays.asList(DirectoryAreas.findAreaById("building")));

    ArrayList<User> usersAdmin = new ArrayList<>(Arrays.asList(new User("Ana", "11343")));

    UserGroup admin = new UserGroup("admin", scheduleAdmin, actionsAdmin, areasAdmin, usersAdmin);
